...
class _ShapeProxy:
    """Lightweight object that provides a .shape attribute without storing data.
    Used as a drop-in replacement for self.train_data_all / self.y_data_all
    when those full tensors would exceed available RAM."""
...
class RetrievalTool():
    ...
    def prepare_dataset(self, train_data):
        """
        Prepare the training dataset by decomposing it into multi-granularity
        representations. Large datasets (e.g. Electricity: 321 channels × 17k samples)
        are processed in batches and stored in float16 to avoid OOM.
        """
        train_data_all = []
        y_data_all = []
        train_data_all_mark = []
        ...
        n_total = len(train_data_all)
        self.n_train = n_total
        
        # Store shape info (needed later for reshape ops).
        sample = train_data_all[0]
        self._train_seq_len = sample.shape[0]
        self._train_channels = sample.shape[1]
        self._y_seq_len = y_data_all[0].shape[0]
        
        # ----- Batch-process train data: stack → decompose → float16 -----
        STACK_BATCH = 512  # batch size for np.stack (memory bottleneck)
        DECOMP_BATCH = 256  # batch size for decompose_mg
        
        mg_train_chunks = []
        for start in range(0, n_total, STACK_BATCH):
            end = min(start + STACK_BATCH, n_total)
            chunk = torch.tensor(np.stack(train_data_all[start:end], axis=0)).float()
            chunk_mg, _ = self.decompose_mg(chunk, batch_t=DECOMP_BATCH)
            mg_train_chunks.append(chunk_mg.half())  # float16 to halve memory
        
        self.train_data_all_mg = torch.cat(mg_train_chunks, dim=1)  # [G, T, S, C] float16
        del mg_train_chunks  # free intermediate list
        
        # ----- Batch-process y (label) data -----
        mg_y_chunks = []
        for start in range(0, n_total, STACK_BATCH):
            end = min(start + STACK_BATCH, n_total)
            chunk = torch.tensor(np.stack(y_data_all[start:end], axis=0)).float()
            chunk_mg, _ = self.decompose_mg(chunk, batch_t=DECOMP_BATCH)
            mg_y_chunks.append(chunk_mg.half())
        
        self.y_data_all_mg = torch.cat(mg_y_chunks, dim=1)  # [G, T, P, C] float16
        del y_data_all, mg_y_chunks
        
        # self.train_data_all / self.y_data_all are NOT stored as full tensors.
        # Their shapes are available via _train_seq_len / _train_channels / _y_seq_len.
        # For backward compatibility, provide a dummy with the correct .shape attribute.
        self.train_data_all = _ShapeProxy(self.n_train, self._train_seq_len, self._train_channels)
        self.y_data_all = _ShapeProxy(self.n_train, self._y_seq_len, self._train_channels)
        
        # Save timestamp features for time-aware retrieval (small, fits in memory).
        ...
        self.dataset_stationarity = self._compute_dataset_stationarity_batched(
            train_data_all, n_total, self._train_seq_len, self._train_channels
        )
        ...
        # Free train_data_all after stationarity computation is done.
        del train_data_all
    ...
    def _compute_dataset_stationarity_batched(self, data_list, n_total, seq_len, channels, batch_size=512):
        """
        Batched version of compute_dataset_stationarity that processes data in
        chunks to avoid OOM on large datasets. Takes a list of numpy arrays
        ...
        for start in range(0, N, batch_size):
            ...
            chunk_np = np.stack(data_list[start:end], axis=0)
            chunk = torch.tensor(chunk_np).float()
            scores = self.compute_stationarity_scores(chunk)
            all_scores.append(scores)
        
        sample_stationarity = torch.cat(all_scores, dim=0)  # [N]
    ...
    def decompose_mg(self, data_all, remove_offset=True, batch_t=512):
        """
        Decompose time-series data into components with different period lengths.
        Processes the T (samples) dimension in batches to avoid OOM on large datasets
        (e.g., Electricity with 321 channels × 17k samples).
        
        Args:
            data_all: [T, S, C] time-series data
            remove_offset: whether to subtract final-step offset
            batch_t: batch size along the T dimension (default 512)
        Returns:
            mg: [G, T, S, C] multi-granularity decomposition
            offset: [G, T, 1, C] or None
        """
        T_total = data_all.shape[0]
        
        mg_chunks = []
        offset_chunks = [] if remove_offset else None
        
        for t_start in range(0, T_total, batch_t):
            t_end = min(t_start + batch_t, T_total)
            chunk = copy.deepcopy(data_all[t_start:t_end])  # [batch_t, S, C]
            
            mg = []
            for g in self.period_num:
                cur = chunk.unfold(dimension=1, size=g, step=g).mean(dim=-1)
                cur = cur.repeat_interleave(repeats=g, dim=1)
                mg.append(cur)
            
            mg = torch.stack(mg, dim=0)  # [G, batch_t, S, C]
            
            if remove_offset:
                offset = []
                for i, data_p in enumerate(mg):
                    cur_offset = data_p[:, -1:, :]  # [batch_t, 1, C]
                    mg[i] = data_p - cur_offset
                    offset.append(cur_offset)
                offset = torch.stack(offset, dim=0)  # [G, batch_t, 1, C]
                offset_chunks.append(offset)
            
            mg_chunks.append(mg)
        
        mg = torch.cat(mg_chunks, dim=1)  # [G, T, S, C]
        
        if remove_offset:
            offset = torch.cat(offset_chunks, dim=1)  # [G, T, 1, C]
        else:
            offset = None
    ...
    def generate_retrieval_predictions(self, sim, bsz):
        ...
        y_data_all = self.y_data_all_mg.flatten(start_dim=2).float() # G, T, P * C
    ...
    def generate_retrieval_predictions_2(self, sim, bsz, index):
        ...
        data_len, seq_len, channels = self.train_data_all.shape
        y_data_all = self.y_data_all_mg.flatten(start_dim=2).float()  # [G, T, P*C] upcast to float32 for bmm
